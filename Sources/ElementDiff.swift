...
extension UICollectionView {
  ...
  public func updateSection(_ section: Int, diff: ElementDiff) {
    ...
    let reloadedIndexPaths = diff.unmoved.map { IndexPath(item: $0, section: section) }
...
extension UITableView {
  ...
  public func updateSection(
    _ section: Int,
    diff: ElementDiff,
    with defaultRowAnimation: UITableViewRowAnimation = UITableViewRowAnimation.automatic,
    ...
    reloadRowAnimation: UITableViewRowAnimation? = nil)
  {
    ...
    let reloadedIndexPaths = diff.unmoved.map { IndexPath(item: $0, section: section) }
    self.reloadRows(at: reloadedIndexPaths, with: reloadRowAnimation ?? defaultRowAnimation)
  }
}
